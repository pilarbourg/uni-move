...
class University:
    ...
    def search_by_degree(degree_to_search: str):
        # Traer todas las universidades con sus grados relacionados
        response = supabase.table("universities").select(
            "id, name, ranking, university_degrees(degree_id, degrees(name))"
        ).execute()
        ...
        results = []
        for row in data:
            for ud in row.get("university_degrees", []):
        ...
        if not results:
            raise DegreeNotFoundException(
                f"No universities found for the degree: {degree_to_search}"
            )
